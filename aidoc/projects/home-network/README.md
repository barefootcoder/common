...
- **Fire-evac cloud backup launched** (May 19 2026): During a shelter-in-place fire warning, triggered a comprehensive Backblaze B2 backup of all at-risk machines (Avalir, Nakama, Zadash). Three parallel `rclone copy --update` runners spawned in detached screen sessions (one per host), each working through a tiered priority queue. New B2 bucket `barefoot-encrypted` created for client-side-encrypted credential material (rclone-crypt). Cleartext B2 keys + crypt passphrase saved to `/export/personal/Dropbox/sensitive/` which Syncthing replicates to Haven (the laptop going with the user). Recovery procedure documented in [fire-evac-recovery.md](fire-evac-recovery.md). Logs in `/var/tmp/fire-backup/` on Avalir+Zadash and `/share/CACHEDEV1_DATA/homes/admin/fire-backup/` on Nakama; status snapshot via `~/bin/fire-backup-status`.
...
- **Haven Crash Triggered by Daily Timeshift Snapshot** (May 17 2026): Silent power-off at 05:00:33 PDT after a 3.5-day uptime — the longest single trigger-to-trigger window of the May series, but still in the established ~3-day endurance band. First crash in the May series where post-May-13 viv-mon data + journalctl together identify a *specific* proximate cause: the daily Timeshift snapshot creation fired by the hourly `--check` cron at 05:00:01. Crash signature cleanly rules out thermal (peak tz4 = 61°C), GPU (rc6 *rose* through failure, gfreq returned to 300MHz after a one-sample blip), and user activity (tp=off, load=0.25 just before the trigger). viv-mon's own poll cadence stretched from 264ms to 2.9s over the final 30 seconds — CPU/IO starvation, not thermal. The 02:00/03:00/04:00 lightweight `--check` fires all produced ~1-second bursts and recovered cleanly; 05:00 was where the daily-snapshot logic actually creates a new rsync copy. Existing config already excludes `/home/buddy/**`, `/root/**`, and (via separate filesystem) `/export/**` — so the Timeshift snapshot was already system-only. Mitigation applied: wrapped `/etc/cron.d/timeshift-hourly` with `ionice -c3 nice -n 19` (the politeness wrappers were on the wrong cron — Timeshift's "daily" 7AM cron has them, but the snapshot was empirically firing from the unwrapped hourly cron at 05:00). See `summary:haven-may17-timeshift-snapshot-crash.md`. Hardware intervention (battery + repaste + cap inspection) remains the pending big lever.
