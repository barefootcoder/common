...
- `summary:vagrant-sandbox-readonly-root-fsck.md` - June 29 2026: the **local Vagrant dev sandbox** (`avalir-dev-sandbox`, NOT quin) came up with root mounted **read-only**, so every `ssh-sandbox` action failed ("Read-only file system": perl-locallib `ambiguous redirect` storm, `/etc`/`/home`/`/tmp` write failures, dead ssh-agent). Cause: a prior unclean VM shutdown set ext4's error flag (`dumpe2fs` = `clean with errors`); with `errors=remount-ro` the FS stays ro until an fsck clears it, and the boot's preen-mode fsck bailed on a corrupted orphan-inode list and dropped to the initramfs BusyBox shell (so `vagrant up` timed out -- but the VM kept running). Two diagnostic red herrings noted (aliased `mount` defeating `grep ' / '`; a broad dmesg grep's 1865 benign hits vs 0 real FS errors). Fix: drive the headless console with `VBoxManage controlvm <vm> screenshotpng` (to `~/docs/ai/screenshots/`, agent-readable) + `keyboardputstring`/`keyboardputscancode 1c 9c`, run `e2fsck -fy /dev/mapper/vagrant--vg-root`, `exit` to resume boot -> clean `rw` root. Doc covers the reusable headless-fsck recipe, the auto-mode classifier gate on `-fy`, why the agent "failure" via `vagrant ssh -c` is a test artifact, data safety (vg-root holds only OS+locallib+dev MySQL; real work is on vboxsf shares), and the `vagrant destroy && up` alternative.
