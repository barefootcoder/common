...
- **package** - The target package to export to.
- **filename** - The apparent filename to export to. Some exporting modules, such as autodie or strictures, care about the filename they are being imported to.
- **line** - The apparent line number to export to. To be combined with the "filename" option.
- **level** - The caller level to export to. This will automatically populate the "package", "filename", and "line" options, making it the easiest most consistent option.
- **version** - A version number to check for the module. The equivalent of specifying the version number on a "use" line.
...
For more craziness of this order, have a look at the article at <http://shadow.cat/blog/matt-s-trout/madness-with-methods> which covers coderef abuse and the "${\...}" syntax.
